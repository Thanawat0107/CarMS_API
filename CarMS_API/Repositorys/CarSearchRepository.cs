...
namespace CarMS_API.Repositorys
{
    public class CarSearchRepository : ISearchableRepository<Car, CarSearchParams>
    {
        public Expression<Func<Car, bool>> BuildFilter(CarSearchParams p)
        {
            ...
            c.CarRegistrationNumber.Contains(p.SearchTerm) ||
            c.CarIdentificationNumber.Contains(p.SearchTerm) ||
            c.EngineNumber.Contains(p.SearchTerm) ||
            c.Color.Contains(p.SearchTerm) ||

            c.Brand.Name.Contains(p.SearchTerm) ||
            c.Seller.IdentityNumber.Contains(p.SearchTerm)) &&
